...
#define mp3buf_size 1024//1.25 * nsamples + 7200;
...
JNIEXPORT jstring JNICALL Java_com_v2soft_AndLib_media_MP3Helper_getVersion
  (JNIEnv * env, jclass c) {
...
{
	short int *buf_l = (*env)->GetDirectBufferAddress(env, buffer_l);
	short int *buf_r = (*env)->GetDirectBufferAddress(env, buffer_r);
	(*lgf).num_channels=1;
	(*lgf).mode = 3;
